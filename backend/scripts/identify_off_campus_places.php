...
echo "=== DRY-RUN: IDENTIFICATION DES 39 LIEUX HORS-CAMPUS À SUPPRIMER ===\n\n";

// Names from places_list.txt (116 places)
$allowedNames = [
...
echo "Total dans base locale: {$dbPlaces->count()}\n";
echo "Total dans places_list.txt: " . count($allowedNames) . "\n\n";

$offCampusPlaces = $dbPlaces->filter(function ($place) use ($allowedNames) {
    return !in_array($place->name, $allowedNames);
});

echo "=== LIEUX HORS-CAMPUS À SUPPRIMER ({$offCampusPlaces->count()}) ===\n\n";
