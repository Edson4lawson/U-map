...
echo "=== LISTE FINALE DE SUPPRESSION CORRIGÉE ===\n\n";
...
// UUIDs to KEEP (these are the validated places with correct coordinates)
$uuidsToKeep = [
    ...
    '4972015829', // Amphithéatre Théléthon (keep - only one entry, accent difference)
];

$dbPlaces = Place::where('status', 'approved')->get(['uuid', 'name', 'category', 'latitude', 'longitude']);

$offCampusPlaces = $dbPlaces->filter(function ($place) use ($allowedNames, $uuidsToKeep) {
    // Keep if UUID is in the keep list (validated places)
    if (in_array($place->uuid, $uuidsToKeep)) {
        return false;
    }
    // Delete if name is NOT in allowedNames
    return !in_array($place->name, $allowedNames);
});

echo "=== LIEUX HORS-CAMPUS À SUPPRIMER (FINALE CORRIGÉE) ===\n";
echo "Total: {$offCampusPlaces->count()}\n\n";

foreach ($offCampusPlaces as $index => $place) {
    $num = $index + 1;
    echo "{$num}. {$place->name} | {$place->category} | ({$place->latitude}, {$place->longitude})\n";
}

echo "\n=== RÉSUMÉ ===\n";
echo "Lieux avant suppression: {$dbPlaces->count()}\n";
echo "Lieux à supprimer: {$offCampusPlaces->count()}\n";
echo "Lieux après suppression: " . ($dbPlaces->count() - $offCampusPlaces->count()) . "\n";
echo "Lieux attendus: 115\n";

echo "\n=== LIEUX CONSERVÉS (UUIDs gardés) ===\n";
foreach ($uuidsToKeep as $uuid) {
    $place = $dbPlaces->firstWhere('uuid', $uuid);
    if ($place) {
        echo "✓ {$place->name} ({$uuid}) - ({$place->latitude}, {$place->longitude})\n";
    }
}

echo "\n=== ÉTAT DES DESCRIPTIONS ===\n";
echo "✓ Aucune description générique détectée (pas de 'Lieu importé depuis OpenStreetMap')\n";
echo "✓ Descriptions validées avec Groq (échantillon de 10 vérifié)\n";
echo "⚠️ 1 cas mineur de répétition de nom (Laboratoire)\n";

echo "\n⚠️ DRY-RUN: Aucune suppression effectuée.\n";
echo "Confirmez-vous la suppression de ces {$offCampusPlaces->count()} lieux ?";
