  ...
  <div v-if="place" class="bg-white dark:bg-gray-900 min-h-screen relative pb-24">
    ...
    <div class="-mt-6 relative z-10 bg-white dark:bg-gray-900 rounded-t-3xl px-6 py-8 shadow-inner animate-slide-up">
       ...
       <div class="fixed bottom-20 left-0 w-full px-6 z-30 pointer-events-none">
          <button @click="goToMap" class="btn-primary w-full flex items-center justify-center gap-2 pointer-events-auto transform active:scale-95">
...
const place = computed(() => {
  const feature = campusData.features.find(p => p.properties.id === parseInt(route.params.id))
...
const goToMap = () => {
  // Pass query param to center map? Or just navigate.
  // Ideally, using state or query param to focus map.
