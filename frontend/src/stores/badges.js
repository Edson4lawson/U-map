    ...
    allBadges: [
      ...
      },
      {
        ...
        condition: (visitedIds, places) => {
            const buIds = places.filter(p => {
                const name = (p.properties?.name || '').toLowerCase()
                return p.properties?.id === 'bu_centrale' || name.includes('bibliothèque') || name.includes('bu')
            }).map(p => p.properties.id.toString())
      ...
      },
      {
        ...
        condition: (visitedIds, places) => {
            const restoIds = places.filter(p => {
                const category = (p.properties?.category || '').toLowerCase()
                const name = (p.properties?.name || '').toLowerCase()
                return category.includes('resto') || category.includes('vie étudiante') || name.includes('resto')
            }).map(p => p.properties.id.toString())
